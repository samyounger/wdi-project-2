...
"use strict";(function(globals){if(!('App'in globals)){globals.App={};}globals.App.navigation=function(){$(".home").on("click",this.barsHome);$(".index").on("click",this.barsIndex);};globals.App.barsHome=function(e){event.preventDefault();$(".barNamesContainer").empty();};globals.App.barsIndex=function(e){event.preventDefault();// Grab url
...
globals.App.removeMarkers=function(){for(var i=0;i<globals.App.markers.length;i++){globals.App.markers[i].setMap(null);}};globals.App.reviewsLoop=function(reviews){for(var i=0;i<reviews.length;i++){$(".favourites").append("\n          <p class=\"review\"><strong>"+reviews[i].author_name+"</strong>   rating => "+reviews[i].rating+"</p>\n          <p class=\"review\">"+reviews[i].text+"</p>\n        ");}};globals.App.addInfoWindowForBar=function(data,marker){var bar=data.json.result;if(typeof this.infoWindow!="undefined")this.infoWindow.close();console.log(data);this.infoWindow=new google.maps.InfoWindow({map:this.map,content:"\n        <form class=\"favourites\">\n          <h3>"+bar.name+"</h3>\n          <img src=\"https://maps.googleapis.com/maps/api/place/photo?maxheight=150&photoreference="+bar.photos[0].photo_reference+"&key=AIzaSyByvlUTw9rHtlxIbic2gCVdhpj-8dK7sTQ\">\n          <p><strong>Phone:</strong> "+bar.formatted_phone_number+"</p>\n          <p class=\"webAddress\"><strong>WebSite:</strong> <a href=\""+bar.website+"\">"+bar.website+"</a></p>\n        </form>"});globals.App.reviewsLoop(bar.reviews);this.infoWindow.open(this.map,marker);this.map.setCenter(marker.getPosition());};globals.App.markerListen=function(marker,place){google.maps.event.addListener(marker,'click',function(){var url="http://localhost:3000/api/bar/"+place.place_id;globals.App.ajaxRequest(url,"get",null,function(data){globals.App.addInfoWindowForBar(data,marker);});});$(".barNamesContainer").append("<li class=\"list-group-item\" id=\""+place.place_id+"\">"+place.name+"</li>");$("#"+place.place_id).on("click",marker,function(){var url="http://localhost:3000/api/bar/"+place.place_id;globals.App.ajaxRequest(url,"get",null,function(data){globals.App.addInfoWindowForBar(data,marker);});});};globals.App.addMarker=function(place){var image={url:place.icon,size:new google.maps.Size(71,71),origin:new google.maps.Point(0,0),anchor:new google.maps.Point(17,34),scaledSize:new google.maps.Size(25,25)};var marker=new google.maps.Marker({map:this.map,position:place.geometry.location,icon:image});globals.App.markers.push(marker);globals.App.markerListen(marker,place);};globals.App.callback=function(results,status){$(".barNamesContainer").empty();if(status==google.maps.places.PlacesServiceStatus.OK){for(var i=0;i<results.length;i++){globals.App.addMarker(results[i]);}}};// Places Setup
...
globals.App.geoLocator=function(){var infoWindow=new google.maps.InfoWindow({map:globals.App.map});if(navigator.geolocation){navigator.geolocation.getCurrentPosition(function(position){var pos={lat:position.coords.latitude,lng:position.coords.longitude};var marker=new google.maps.Marker({map:globals.App.map,position:pos});globals.App.map.setCenter(pos);globals.App.placesSetup(pos);},function(){handleLocationError(true,infoWindow,infoWindow.map.getCenter());});}else{// Browser doesn't support Geolocation
