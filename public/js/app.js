...
"use strict";(function(globals){if(!('App'in globals)){globals.App={};}globals.App.navigation=function(){$(".home").on("click",this.barsHome);$(".bars").on("click",this.barsIndex);};globals.App.barsHome=function(e){event.preventDefault();$(".barNamesContainer").empty();};globals.App.barsIndex=function(e){event.preventDefault();// Grab url
var url=$(this).attr("href");// Make the ajax request for all the restaurants
return globals.App.getBarsForIndex(url);};globals.App.getBarsForIndex=function(url){return $.ajax({method:"GET",url:""+this.api_url+url,beforeSend:this.setRequestHeader.bind(this)}).done(this.listBars);};globals.App.listBars=function(data){$(".barNamesContainer").empty();$(".barNamesContainer").prepend("<div class=\"bars\">\n        <h2>Favourite Bars</h2>\n      </div>\n    ");$(".barNamesContainer").prepend($.each(data.bars,function(i,bar){$(".barNamesContainer").append("\n          <h3>"+bar.name+"</h3>\n        ");}));};// globals.App.ajaxRequest = function(url, method, data, callback){
//   return $.ajax({
//     url,
//     method,
//     data,
...
globals.App.removeMarkers=function(){for(var i=0;i<globals.App.markers.length;i++){globals.App.markers[i].setMap(null);}};globals.App.reviewsLoop=function(reviews){for(var i=0;i<reviews.length;i++){$(".favourites").append("\n        <p class=\"review\"><strong>"+reviews[i].author_name+"</strong>   rating => "+reviews[i].rating+"</p>\n        <p class=\"review\">"+reviews[i].text+"</p>\n        ");}};globals.App.addInfoWindowForBar=function(data,marker){var bar=data.json.result;globals.App.barSelected={name:bar.name,googlePlaceId:bar.place_id,url:bar.website,lat:bar.geometry.location.lat,lng:bar.geometry.location.lng};if(typeof this.infoWindow!="undefined")this.infoWindow.close();this.infoWindow=new google.maps.InfoWindow({map:this.map,content:"\n        <form class=\"favourites\">\n          <h3>"+bar.name+"</h3>\n          <div class=\"imageContainer\"></div>\n          <p><strong>Phone:</strong> "+bar.formatted_phone_number+"</p>\n          <p class=\"webAddress\"><strong>WebSite:</strong> <a href=\""+bar.website+"\">"+bar.website+"</a></p>\n          <input type=\"submit\" id=\"favourite\" value=\"Favourite\">\n        </form>"});if(bar.reviews){globals.App.reviewsLoop(bar.reviews);}if(bar.photos){$(".imageContainer").append("<img class=\"barImage\" src=\"https://maps.googleapis.com/maps/api/place/photo?maxheight=150&photoreference="+bar.photos[0].photo_reference+"&key=AIzaSyByvlUTw9rHtlxIbic2gCVdhpj-8dK7sTQ\">");}this.infoWindow.open(this.map,marker);this.map.setCenter(marker.getPosition());};globals.App.markerListen=function(marker,place){google.maps.event.addListener(marker,'click',function(){var url="http://localhost:3000/api/bar/"+place.place_id;globals.App.ajaxRequest(url,"get",null,function(data){globals.App.addInfoWindowForBar(data,marker);});});$(".barNamesContainer").append("<li class=\"list-group-item\" id=\""+place.place_id+"\"><a href=\"#\">"+place.name+"</a></li>");$("#"+place.place_id).on("click",marker,function(){var url="http://localhost:3000/api/bar/"+place.place_id;globals.App.ajaxRequest(url,"get",null,function(data){globals.App.addInfoWindowForBar(data,marker);});});};globals.App.addMarker=function(place){var image={url:place.icon,size:new google.maps.Size(71,71),origin:new google.maps.Point(0,0),anchor:new google.maps.Point(17,34),scaledSize:new google.maps.Size(25,25)};var marker=new google.maps.Marker({map:this.map,position:place.geometry.location,icon:image});globals.App.markers.push(marker);globals.App.markerListen(marker,place);};globals.App.callback=function(results,status){$(".barNamesContainer").empty();if(status==google.maps.places.PlacesServiceStatus.OK){for(var i=0;i<results.length;i++){globals.App.addMarker(results[i]);}}};// Places Setup
...
};globals.App.saveBar=function(){event.preventDefault();console.log("clicked");var method="POST";var url=globals.App.api_url+"/bars/"+globals.App.getId();var data=globals.App.barSelected;return $.ajax({method:method,url:url,data:data,beforeSend:globals.App.setRequestHeader.bind(this)}).done(function(data){return getBarsIndex("/bars");}).fail(function(data){console.log(data);});};function getBarsIndex(url){return $.ajax({method:"GET",url:""+globals.App.api_url+url}).done(listBars);}// To be updated
